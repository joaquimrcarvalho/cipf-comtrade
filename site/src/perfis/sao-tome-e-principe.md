# Perfil: São Tomé e Príncipe

Perfil comercial de **São Tomé e Príncipe** (2003–2024), espelhando a estrutura do bloco de
...
const balance = await FileAttachment("../data/sao-tome-e-principe_trade_balance_2003-2024.csv").csv({typed: true});
const topPartX = await FileAttachment("../data/sao-tome-e-principe_top_partners_exports_2003-2024.csv").csv({typed: true});
const topPartM = await FileAttachment("../data/sao-tome-e-principe_top_partners_imports_2003-2024.csv").csv({typed: true});
const topProdX = await FileAttachment("../data/sao-tome-e-principe_top_products_exports_HS-AG6_2003-2024.csv").csv({typed: true});
const topProdM = await FileAttachment("../data/sao-tome-e-principe_top_products_imports_HS-AG6_2003-2024.csv").csv({typed: true});
const d8x = await FileAttachment("../data/sao-tome-e-principe_products_partners_HS-AG6_2003-2024.csv").csv({typed: true});
const d8m = await FileAttachment("../data/sao-tome-e-principe_partners_products_HS-AG6_2003-2024.csv").csv({typed: true});
const meta = await FileAttachment("../data/sao-tome-e-principe_profile_2003-2024.meta.json").json();
```
...
<div class="note" style="font-size: 0.85rem; color: var(--theme-foreground-muted)">
  ...
  valores <em>diretos</em> reportados por São Tomé e Príncipe; valores <em>espelho</em> reportados
...
const basis = view(Inputs.radio(
  new Map([
    ["Reportado por São Tomé e Príncipe (direto)", "direct"],
...
display(kpiCards(html, [
  {label: "Volume de trocas", value: formatUSD(at(latest, "trade_volume")),
   sub: `São Tomé e Príncipe ↔ mundo, ${latestYear}`},
  {label: "Exportações", value: formatUSD(at(latest, "exports")), sub: "São Tomé e Príncipe → mundo"},
  {label: "Importações", value: formatUSD(at(latest, "imports")), sub: "mundo → São Tomé e Príncipe"},
  {label: "Saldo comercial", value: formatUSD(at(latest, "balance")), sub: "perspetiva de São Tomé e Príncipe"}
]));
```
...
## 2. Exportações de São Tomé e Príncipe
...
Para os principais clientes de São Tomé e Príncipe e os principais produtos exportados: a quota de São Tomé e Príncipe e dos outros principais fornecedores nas importações de cada cliente — a posição de São Tomé e Príncipe entre os fornecedores do cliente (bloco de notas §2.5; base direta).

```js
const compX = await FileAttachment("../data/sao-tome-e-principe_competition_exports_HS-AG6_2003-2024.csv").csv({typed: true});
```
...
<div style="font-size: 0.85rem; color: var(--theme-foreground-muted)">
  Linha de São Tomé e Príncipe realçada; a tabela mostra a posição no último ano com dados deste
  mercado. Cobertura: 5 principais clientes × 8 principais produtos exportados; até 5 concorrentes por mercado, além de São Tomé e Príncipe.
</div>


## 3. Importações de São Tomé e Príncipe
...
<div style="font-size: 0.85rem; color: var(--theme-foreground-muted)">
  ${L.fonte} · base: ${basis === "direct" ? "valores reportados por São Tomé e Príncipe" : "valores reportados pelos parceiros (espelho)"} ·
...
Para os principais fornecedores de São Tomé e Príncipe e os principais produtos importados: a quota de São Tomé e Príncipe e dos outros principais clientes nas exportações de cada fornecedor — a posição de São Tomé e Príncipe entre os clientes do fornecedor (bloco de notas §3.5; base direta).

```js
const compM = await FileAttachment("../data/sao-tome-e-principe_competition_imports_HS-AG6_2003-2024.csv").csv({typed: true});
```
...
<div style="font-size: 0.85rem; color: var(--theme-foreground-muted)">
  Linha de São Tomé e Príncipe realçada; a tabela mostra a posição no último ano com dados deste
  mercado. Cobertura: 5 principais fornecedores × 8 principais produtos importados; até 5 outros clientes por mercado, além de São Tomé e Príncipe.
</div>
...
```js
display(html`<p>
  <a href="${FileAttachment("../data/sao-tome-e-principe_trade_balance_2003-2024.csv").href}" download>Balança comercial (CSV)</a> ·
  <a href="${FileAttachment("../data/sao-tome-e-principe_top_partners_exports_2003-2024.csv").href}" download>Clientes (CSV)</a> ·
  <a href="${FileAttachment("../data/sao-tome-e-principe_top_partners_imports_2003-2024.csv").href}" download>Fornecedores (CSV)</a> ·
  <a href="${FileAttachment("../data/sao-tome-e-principe_top_products_exports_HS-AG6_2003-2024.csv").href}" download>Produtos exportados (CSV)</a> ·
  <a href="${FileAttachment("../data/sao-tome-e-principe_top_products_imports_HS-AG6_2003-2024.csv").href}" download>Produtos importados (CSV)</a> ·
  <a href="${FileAttachment("../data/sao-tome-e-principe_products_partners_HS-AG6_2003-2024.csv").href}" download>Produto × parceiro (CSV)</a> ·
  <a href="${FileAttachment("../data/sao-tome-e-principe_partners_products_HS-AG6_2003-2024.csv").href}" download>Parceiro × produto (CSV)</a> ·
  <a href="${FileAttachment("../data/sao-tome-e-principe_competition_exports_HS-AG6_2003-2024.csv").href}" download>Concorrência nos clientes (CSV)</a> ·
  <a href="${FileAttachment("../data/sao-tome-e-principe_competition_imports_HS-AG6_2003-2024.csv").href}" download>Concorrência pelos fornecedores (CSV)</a> ·
  <a href="${FileAttachment("../data/sao-tome-e-principe_profile_2003-2024.meta.json").href}" download>${L.downloadMeta}</a>
</p>`);
```
