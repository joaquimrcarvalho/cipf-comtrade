...
Perfil comercial de **Guiné-Bissau** (2003–2024), espelhando a estrutura do bloco de
...
const topPartM = await FileAttachment("../data/guine-bissau_top_partners_imports_2003-2024.csv").csv({typed: true});
const topProdX = await FileAttachment("../data/guine-bissau_top_products_exports_HS-AG6_2003-2024.csv").csv({typed: true});
const topProdM = await FileAttachment("../data/guine-bissau_top_products_imports_HS-AG6_2003-2024.csv").csv({typed: true});
const d8x = await FileAttachment("../data/guine-bissau_products_partners_HS-AG6_2003-2024.csv").csv({typed: true});
const d8m = await FileAttachment("../data/guine-bissau_partners_products_HS-AG6_2003-2024.csv").csv({typed: true});
const meta = await FileAttachment("../data/guine-bissau_profile_2003-2024.meta.json").json();
```
...
<div class="note" style="font-size: 0.85rem; color: var(--theme-foreground-muted)">
  ...
  valores <em>diretos</em> reportados por Guiné-Bissau; valores <em>espelho</em> reportados
...
const basis = view(Inputs.radio(
  new Map([
    ["Reportado por Guiné-Bissau (direto)", "direct"],
...
display(kpiCards(html, [
  {label: "Volume de trocas", value: formatUSD(at(latest, "trade_volume")),
   sub: `Guiné-Bissau ↔ mundo, ${latestYear}`},
  {label: "Exportações", value: formatUSD(at(latest, "exports")), sub: "Guiné-Bissau → mundo"},
  {label: "Importações", value: formatUSD(at(latest, "imports")), sub: "mundo → Guiné-Bissau"},
  {label: "Saldo comercial", value: formatUSD(at(latest, "balance")), sub: "perspetiva de Guiné-Bissau"}
]));
```
...
Para os principais clientes de Guiné-Bissau e os principais produtos exportados: a quota de Guiné-Bissau e dos outros principais fornecedores nas importações de cada cliente — a posição de Guiné-Bissau entre os fornecedores do cliente (bloco de notas §2.5; base direta).

```js
const compX = await FileAttachment("../data/guine-bissau_competition_exports_HS-AG6_2003-2024.csv").csv({typed: true});
```
...
Para os principais fornecedores de Guiné-Bissau e os principais produtos importados: a quota de Guiné-Bissau e dos outros principais clientes nas exportações de cada fornecedor — a posição de Guiné-Bissau entre os clientes do fornecedor (bloco de notas §3.5; base direta).

```js
const compM = await FileAttachment("../data/guine-bissau_competition_imports_HS-AG6_2003-2024.csv").csv({typed: true});
```
...
<div style="font-size: 0.85rem; color: var(--theme-foreground-muted)">
  Linha de Guiné-Bissau realçada; a tabela mostra a posição no último ano com dados deste
  mercado. Cobertura: 5 principais fornecedores × 8 principais produtos importados; até 5 outros clientes por mercado, além de Guiné-Bissau.
</div>
...
```js
display(html`<p>
  <a href="${FileAttachment("../data/guine-bissau_trade_balance_2003-2024.csv").href}" download>Balança comercial (CSV)</a> ·
  <a href="${FileAttachment("../data/guine-bissau_top_partners_exports_2003-2024.csv").href}" download>Clientes (CSV)</a> ·
  <a href="${FileAttachment("../data/guine-bissau_top_partners_imports_2003-2024.csv").href}" download>Fornecedores (CSV)</a> ·
  <a href="${FileAttachment("../data/guine-bissau_top_products_exports_HS-AG6_2003-2024.csv").href}" download>Produtos exportados (CSV)</a> ·
  <a href="${FileAttachment("../data/guine-bissau_top_products_imports_HS-AG6_2003-2024.csv").href}" download>Produtos importados (CSV)</a> ·
  <a href="${FileAttachment("../data/guine-bissau_products_partners_HS-AG6_2003-2024.csv").href}" download>Produto × parceiro (CSV)</a> ·
  <a href="${FileAttachment("../data/guine-bissau_partners_products_HS-AG6_2003-2024.csv").href}" download>Parceiro × produto (CSV)</a> ·
  <a href="${FileAttachment("../data/guine-bissau_competition_exports_HS-AG6_2003-2024.csv").href}" download>Concorrência nos clientes (CSV)</a> ·
  <a href="${FileAttachment("../data/guine-bissau_competition_imports_HS-AG6_2003-2024.csv").href}" download>Concorrência pelos fornecedores (CSV)</a> ·
  <a href="${FileAttachment("../data/guine-bissau_profile_2003-2024.meta.json").href}" download>${L.downloadMeta}</a>
</p>`);
```
