...
Perfil comercial de **Portugal** (2003–2024), espelhando a estrutura do bloco de
...
const balance = await FileAttachment("../data/portugal_trade_balance_2003-2024.csv").csv({typed: true});
const topPartX = await FileAttachment("../data/portugal_top_partners_exports_2003-2024.csv").csv({typed: true});
const topPartM = await FileAttachment("../data/portugal_top_partners_imports_2003-2024.csv").csv({typed: true});
const topProdX = await FileAttachment("../data/portugal_top_products_exports_HS-AG6_2003-2024.csv").csv({typed: true});
const topProdM = await FileAttachment("../data/portugal_top_products_imports_HS-AG6_2003-2024.csv").csv({typed: true});
const d8x = await FileAttachment("../data/portugal_products_partners_HS-AG6_2003-2024.csv").csv({typed: true});
const d8m = await FileAttachment("../data/portugal_partners_products_HS-AG6_2003-2024.csv").csv({typed: true});
const meta = await FileAttachment("../data/portugal_profile_2003-2024.meta.json").json();
```
...
<div class="note" style="font-size: 0.85rem; color: var(--theme-foreground-muted)">
  ...
  valores <em>diretos</em> reportados por Portugal; valores <em>espelho</em> reportados
...
const basis = view(Inputs.radio(
  new Map([
    ["Reportado por Portugal (direto)", "direct"],
...
display(kpiCards(html, [
  {label: "Volume de trocas", value: formatUSD(at(latest, "trade_volume")),
   sub: `Portugal ↔ mundo, ${latestYear}`},
  {label: "Exportações", value: formatUSD(at(latest, "exports")), sub: "Portugal → mundo"},
  {label: "Importações", value: formatUSD(at(latest, "imports")), sub: "mundo → Portugal"},
  {label: "Saldo comercial", value: formatUSD(at(latest, "balance")), sub: "perspetiva de Portugal"}
]));
```
...
Para os principais clientes de Portugal e os principais produtos exportados: a quota de Portugal e dos outros principais fornecedores nas importações de cada cliente — a posição de Portugal entre os fornecedores do cliente (bloco de notas §2.5; base direta).

```js
const compX = await FileAttachment("../data/portugal_competition_exports_HS-AG6_2003-2024.csv").csv({typed: true});
```
...
<div style="font-size: 0.85rem; color: var(--theme-foreground-muted)">
  Linha de Portugal realçada; a tabela mostra a posição no último ano com dados deste
  mercado. Cobertura: 5 principais clientes × 8 principais produtos exportados; até 5 concorrentes por mercado, além de Portugal.
</div>


## 3. Importações de Portugal
...
<div style="font-size: 0.85rem; color: var(--theme-foreground-muted)">
  ${L.fonte} · base: ${basis === "direct" ? "valores reportados por Portugal" : "valores reportados pelos parceiros (espelho)"} ·
...
Para os principais fornecedores de Portugal e os principais produtos importados: a quota de Portugal e dos outros principais clientes nas exportações de cada fornecedor — a posição de Portugal entre os clientes do fornecedor (bloco de notas §3.5; base direta).

```js
const compM = await FileAttachment("../data/portugal_competition_imports_HS-AG6_2003-2024.csv").csv({typed: true});
```
...
<div style="font-size: 0.85rem; color: var(--theme-foreground-muted)">
  Linha de Portugal realçada; a tabela mostra a posição no último ano com dados deste
  mercado. Cobertura: 5 principais fornecedores × 8 principais produtos importados; até 5 outros clientes por mercado, além de Portugal.
</div>
...
```js
display(html`<p>
  <a href="${FileAttachment("../data/portugal_trade_balance_2003-2024.csv").href}" download>Balança comercial (CSV)</a> ·
  <a href="${FileAttachment("../data/portugal_top_partners_exports_2003-2024.csv").href}" download>Clientes (CSV)</a> ·
  <a href="${FileAttachment("../data/portugal_top_partners_imports_2003-2024.csv").href}" download>Fornecedores (CSV)</a> ·
  <a href="${FileAttachment("../data/portugal_top_products_exports_HS-AG6_2003-2024.csv").href}" download>Produtos exportados (CSV)</a> ·
  <a href="${FileAttachment("../data/portugal_top_products_imports_HS-AG6_2003-2024.csv").href}" download>Produtos importados (CSV)</a> ·
  <a href="${FileAttachment("../data/portugal_products_partners_HS-AG6_2003-2024.csv").href}" download>Produto × parceiro (CSV)</a> ·
  <a href="${FileAttachment("../data/portugal_partners_products_HS-AG6_2003-2024.csv").href}" download>Parceiro × produto (CSV)</a> ·
  <a href="${FileAttachment("../data/portugal_competition_exports_HS-AG6_2003-2024.csv").href}" download>Concorrência nos clientes (CSV)</a> ·
  <a href="${FileAttachment("../data/portugal_competition_imports_HS-AG6_2003-2024.csv").href}" download>Concorrência pelos fornecedores (CSV)</a> ·
  <a href="${FileAttachment("../data/portugal_profile_2003-2024.meta.json").href}" download>${L.downloadMeta}</a>
</p>`);
```
