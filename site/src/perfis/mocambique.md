...
Perfil comercial de **Moçambique** (2003–2024), espelhando a estrutura do bloco de
...
const topPartM = await FileAttachment("../data/mocambique_top_partners_imports_2003-2024.csv").csv({typed: true});
const topProdX = await FileAttachment("../data/mocambique_top_products_exports_HS-AG6_2003-2024.csv").csv({typed: true});
const topProdM = await FileAttachment("../data/mocambique_top_products_imports_HS-AG6_2003-2024.csv").csv({typed: true});
const d8x = await FileAttachment("../data/mocambique_products_partners_HS-AG6_2003-2024.csv").csv({typed: true});
const d8m = await FileAttachment("../data/mocambique_partners_products_HS-AG6_2003-2024.csv").csv({typed: true});
const meta = await FileAttachment("../data/mocambique_profile_2003-2024.meta.json").json();
```
...
<div class="note" style="font-size: 0.85rem; color: var(--theme-foreground-muted)">
  ...
  valores <em>diretos</em> reportados por Moçambique; valores <em>espelho</em> reportados
...
const basis = view(Inputs.radio(
  new Map([
    ["Reportado por Moçambique (direto)", "direct"],
...
display(kpiCards(html, [
  {label: "Volume de trocas", value: formatUSD(at(latest, "trade_volume")),
   sub: `Moçambique ↔ mundo, ${latestYear}`},
  {label: "Exportações", value: formatUSD(at(latest, "exports")), sub: "Moçambique → mundo"},
  {label: "Importações", value: formatUSD(at(latest, "imports")), sub: "mundo → Moçambique"},
  {label: "Saldo comercial", value: formatUSD(at(latest, "balance")), sub: "perspetiva de Moçambique"}
]));
```
...
<div style="font-size: 0.85rem; color: var(--theme-foreground-muted)">
  ${L.fonte} · base: ${basis === "direct" ? "valores reportados por Moçambique" : "valores reportados pelos parceiros (espelho)"} ·
...
```js
display(html`<p>
  <a href="${FileAttachment("../data/mocambique_trade_balance_2003-2024.csv").href}" download>Balança comercial (CSV)</a> ·
  <a href="${FileAttachment("../data/mocambique_top_partners_exports_2003-2024.csv").href}" download>Clientes (CSV)</a> ·
  <a href="${FileAttachment("../data/mocambique_top_partners_imports_2003-2024.csv").href}" download>Fornecedores (CSV)</a> ·
  <a href="${FileAttachment("../data/mocambique_top_products_exports_HS-AG6_2003-2024.csv").href}" download>Produtos exportados (CSV)</a> ·
