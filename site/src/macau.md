# Trocas Macau (RAEM)–PLP

Evolução das trocas comerciais entre **Macau (RAEM)** e os Países de Língua Portuguesa
(PLP), e do papel de Macau como **plataforma** entre a China e o mundo lusófono.
Os valores *diretos* são reportados por Macau; os valores *espelho* são inferidos dos
reportes dos parceiros — divergências são esperadas (CIF/FOB, reexportações, falhas de
reporte) e ambas as bases são apresentadas.
...
const flows = await FileAttachment("data/mo_plp_flows_2003-2024.csv").csv({typed: true});
const meta = await FileAttachment("data/mo_plp_flows_2003-2024.meta.json").json();
const cnFlows = await FileAttachment("data/cn_plp_flows_2003-2024.csv").csv({typed: true});
```
...
display(kpiCards(html, [
  {label: "Trocas comerciais", value: formatUSD(sumLatest("trade_volume")), sub: `Macau (RAEM) ↔ PLP, ${latestYear}`},
  {label: "Exportações de Macau", value: formatUSD(sumLatest("exports")), sub: "para os PLP"},
  {label: "Importações de Macau", value: formatUSD(sumLatest("imports")), sub: "dos PLP"},
  {label: "Saldo comercial", value: formatUSD(sumLatest("balance")), sub: "perspetiva de Macau"}
]));
```

## Macau como plataforma

Que peso têm as trocas diretas de Macau com os PLP face ao fluxo China ↔ PLP?
A linha mostra o volume de trocas Macau–PLP como **percentagem** do volume China–PLP
(ambos em base direta).

```js
const cnByYear = new Map(
  d3.rollups(cnFlows.filter((d) => d.basis === "direct"),
    (v) => d3.sum(v, (d) => d.trade_volume), (d) => d.year)
);
const share = d3
  .rollups(flows.filter((d) => d.basis === "direct"),
    (v) => d3.sum(v, (d) => d.trade_volume), (d) => d.year)
  .map(([year, mo]) => ({year, share: (mo / cnByYear.get(year)) * 100}))
  .filter((d) => Number.isFinite(d.share))
  .sort((a, b) => d3.ascending(a.year, b.year));
const latestShare = share.at(-1);
```

Em ${latestShare.year}, as trocas Macau–PLP corresponderam a **${pct(latestShare.share)}**
do volume China–PLP.
...
display(Plot.plot({
  marginLeft: 60,
  grid: true,
  y: {label: "Macau–PLP como % do volume China–PLP", tickFormat: (v) => pct(v)},
  x: {label: null, tickFormat: "d"},
  marks: [
    Plot.areaY(share, {x: "year", y: "share", fillOpacity: 0.12}),
    Plot.line(share, {x: "year", y: "share", strokeWidth: 2, tip: {
      format: {x: "d", y: (v) => pct(v, 2)}
    }}),
    Plot.dot(share, {x: "year", y: "share", r: 2})
  ]
}));
```
...
const basis = view(Inputs.radio(
  new Map([
    ["Reportado por Macau (RAEM)", "direct"],
...
<div style="font-size: 0.85rem; color: var(--theme-foreground-muted)">
  ${L.fonte} · base: ${basis === "direct" ? "valores reportados por Macau (RAEM)" : "valores reportados pelos parceiros (espelho)"} ·
...
```js
display(html`<p>
  <a href="${FileAttachment("data/mo_plp_flows_2003-2024.csv").href}" download>${L.downloadCSV}</a> ·
  <a href="${FileAttachment("data/mo_plp_flows_2003-2024.meta.json").href}" download>${L.downloadMeta}</a>
</p>`);
```

---

Dados de origem: bloco de notas `mo_plp_import_export.ipynb` do projeto
