...
- **No Taiwan (Prov. China) data on the public site** (editorial decision, 2026-07-19);
  the Taiwan analysis remains available in the repository notebooks and `reports/`.
...
| D4 | Taiwan (Prov. China) ↔ PLP flows — **excluded from the public site** (editorial decision, 2026-07-19; analysis stays in the notebooks/reports) | `tw_plp_import_export.ipynb` | year × 9 PLPs | `reports/tw_plp_*` |
...
│   ├── package.json            # (no observablehq.config.* — Framework defaults)
│   ├── src/
│   │   ├── index.md                # landing page / dashboard
│   │   ├── china-plp.md            # D1 explorer
│   │   ├── macau.md hong-kong.md   # D2/D3 explorers (D4 Taiwan: notebooks only)
│   │   ├── perfis/<slug>.md        # country profiles (D5–D8), generated
│   │   ├── metodologia.md          # methodology & sources
│   │   ├── data/                   # committed data snapshots (§5)
│   │   └── components/             # shared JS chart components (import-free by design)
│   └── scripts/
│       ├── export_site_data.py     # flows export: comtradetools → site/src/data
│       ├── export_profiles.py      # D5–D8 export (resumable, adaptive chunking)
│       └── gen_profile_pages.py    # generates src/perfis/<slug>.md from a template
...
- Snapshots are regenerated by `export_site_data.py` (flows D1–D3) and
  `export_profiles.py` (country profiles D5–D8), which **fail loudly** on empty
...
**`<slug>_trade_balance_2003-YYYY.csv`** (D5; slugs like `angola`, `cabo-verde`)

Long format (consistent with D1): `year, basis, exports, imports, trade_volume, balance`
— one row per (year, basis), `basis ∈ {direct, mirror}`, zeros where a flow/year is
unreported. Covers the full period for every country.

**`<slug>_top_partners_exports_2003-YYYY.csv`** (D6; `_imports_` variant)

`year, partner_code, partner, value, share_pct, rank, basis` — top-10 partners per
(year, basis); `share_pct` vs the year's world total of the same basis; `partner` in
Portuguese (curated dictionary, Comtrade English fallback).

**`<slug>_top_products_exports_HS-AG6_2003-YYYY.csv`** (D7; `_imports_` variant)

`year, hs6, description_pt, value, share_pct, rank, basis` — top-10 products per
(year, basis); `share_pct` vs the year's world TOTAL (same basis — note AG6 detail
may not sum to TOTAL because of confidentiality suppressions);
`description_pt`: curated PT label, else the English HS description.

**`<slug>_products_partners_HS-AG6_2003-YYYY.csv`** (D8 exports; imports variant
`<slug>_partners_products_HS-AG6_...`)
...
carry **no** `World` reporter row and `reporterCode=0` returns empty. The World row
is therefore never mixed into an aggregate, and the double counting a naive sum
could risk cannot occur. Rankings never aggregate beyond the reported partners.

**AG6 fetch strategy (D7/D8):** two stages keep responses far below the API record
cap — (1) D7 **direct** discovery uses world-level queries (`reporter=C, partner=0`);
(2) for **strong direct reporters** (direct AG6 covering ≥ 15 of the period's
years) the mirror product views derive from the D8 mirror frame, and D8 detail
queries are restricted to the all-time top-25 direct HS6 codes via `cmdCode` CSV
lists — avoiding full-detail mirror AG6 fetches entirely; (3) for **weak direct
reporters** (e.g. long non-reporting gaps) top products are discovered from a full
mirror AG6 fetch (small for those countries) and the D8 code set is the union over
bases. Independently, every fetched chunk is checked against the per-call record
cap (≥ 99 000 rows = suspected truncation) and only suspicious chunks are refetched
year by year (`fetch_guarded` in `export_profiles.py`; period splitting and
caching stay inside `comtradetools.getFinalData`).

**Reference:** `plp_countries.json` (M49, PT/EN names), `hs_ag6_labels.json`
(HS6 → `{en, pt}` for every code emitted in D7/D8).
...
| `/macau`, `/hong-kong` | Macau (RAEM) / Hong Kong (RAEHK)–PLP | D2–D3 | Same explorer pattern; Macau (RAEM) page adds "platform role" narrative comparing Macau (RAEM)'s share vs. China direct |
...
| **2 — Platforms (D2–D3)** | Macau (RAEM)/HK (RAEHK) pages, Macau (RAEM)-platform narrative | Same checks; navigation complete |
